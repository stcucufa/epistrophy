...
// or nothing if the value could not be parsed (with a warning).
export function parseOffsetValue(offset) {
    ...
    console.warn(`Could not parse offset value "${offset}"`);
}

// Render an offset (in millisecond) as a clock value.
export function clockValue(offset) {
    const ms = Math.round(offset);
    const pad = n => Math.floor(n).toString().padStart(2, "0");
    return `${Math.floor(offset / Hour)}:${pad(offset / Minute)}:${pad(offset / Second)}.${
