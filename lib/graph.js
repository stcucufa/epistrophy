...
export default class Graph {
    ...
    addNode(node) {
        ...
        this.scheduler.scheduleFiber(node.layout(
            new Fiber().call(({ id, scheduler: { now } }) => { console.log(`<<< ${id}: ${now}`); })
        ).call(({ id, value, scheduler: { now } }) => { console.log(`>>> ${id}: ${now}`, value); }), node.begin);
        return node;
    }
...
// A Call represents a sync function call.
...
class Element extends Node {
    constructor(tag, attributes = {}) {
        ...
        for (const p of BaseAttributes) {
            this.attributes.delete(p);
        }
    }
