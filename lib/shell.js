...
Fiber.prototype.repeat = function(f, delegate) {
    ...
    this.seq(fiber => fiber.
        call(fiber => {
            if (delegate?.repeatShouldEnd?.call(delegate, 0, fiber)) {
                return;
            }
        ...
        join({
            ...
            childFiberDidJoin(child) {
                delegate?.childFiberDidJoin?.call(delegate, child);
                ...
                if ((fiber.error && !fiber.handlesErrors) ||
                    delegate?.repeatShouldEnd?.call(delegate, this.i, fiber)) {
                    return;
                }
                fiber.scheduler.attachFiber(fiber, body);
            }
    ...
    this.seq(fiber => fiber.
        ...
        join({
            ...
            childFiberDidJoin(child) {
                ...
                fiber.scheduler.attachFiberWithValue(fiber, body, fiber.value[this.i]);
            }
        })
    );
    if (typeof f === "function") {
...
Fiber.prototype.mapspawn = function(f) {
    ...
    return this.call(fiber => {
        for (const value of fiber.value) {
            fiber.scheduler.attachFiber(fiber, body).value = value;
        }
    });
};
