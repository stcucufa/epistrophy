...
// Shorthand for sync(K(x)); set the value of the fiber to a constant.
Fiber.prototype.K = function(x) {
    return this.sync(K(x));
};
...
// (Array) value of the fiber, without joining.
Fiber.prototype.each = function(f) {
    const body = new Fiber();
    f(body);
    return this.sync((fiber, scheduler) => {
...
// Create a new fiber populated with `f`, and spawn it for evert item of the
// (Array) value of the fiber, then join, collecting the value of each child
...
Fiber.prototype.maporder = function(f) {
    ...
    f(body);
    return this.
        sync((fiber, scheduler) => fiber.value.map(v => {
            const scheduledFiber = scheduler.attachFiber(fiber, body);
            scheduledFiber.value = v;
...
Fiber.prototype.mapfirst = function(f) {
    ...
    return this.
        sync((fiber, scheduler) => {
            for (const x of fiber.value) {
                scheduler.attachFiber(fiber, body).value = x;
            }
        }).
        join(FirstValue);
}
