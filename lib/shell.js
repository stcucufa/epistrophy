...
import { Fiber, ScheduledFiber, Scheduler, Arg } from "./kernel.js";
...
Fiber.prototype.rate = function(rate) {
    return this.call(fiber => {
        // Push the previous rate in the trace to restore it when undoing,
        // unless there is a change of direction, in which case the new rate
        // is pushed so that undoing has no effect.
        fiber.trace.at(-1).push(fiber.rate * rate < 0 ? rate : fibrer.rate);
        fiber.scheduler.setRateForFiber(fiber, rate);
    }, fiber => {
        fiber.scheduler.setRateForFiber(fiber, fiber.trace.at(-1)[Arg]);
    });
};
...
Fiber.prototype.scale = function(scale) {
    return this.rate(1 / scale);
};

// Spawn a fiber that loops indefinitely; i.e., that jumps back to its first
// instruction as soon as it reaches its last.
Fiber.prototype.loop = function(f, delegate) {
    this.spawn(fiber => fiber.
        call(fiber => { fiber.loopDelegate = extend(delegate, { iterationCount: 0 }); }).
        call(fiber => {
            const end = fiber.fiber.length;
            try {
                if (fiber.loopDelegate.loopShouldEnd?.(fiber)) {
                    fiber.ip = end;
                }
            } catch (error) {
        ...
        append(f).
        call(fiber => {
            fiber.loopDelegate.iterationCount += 1;
            fiber.ip = 1;
        })
    );
    return delegate ? this.join(Nearest) : this;
};

// Join delegate for loops.
export const Nearest = {
    fiberWillJoin(fiber) {
        this.nearestFiber = fiber.children.at(-1);
    },
    finalChildFiberDidJoin(child) {
        if (child !== this.nearestFiber) {
            return;
        }
...
// FIXME 550A Shell: remove repeat
...
// FIXME 5605 Shell: cancelSiblings should be a method of ScheduledFiber
...
// Reset the ramp duration for a fiber, i.e., add the duration to the elapsed
// time for the new duration.
Scheduler.prototype.resetRampDurationForFiber = function(fiber, dur) {
    if (!(dur >= 0)) {
        throw Error("Ramp duration is not a positive number");
    }
    if (fiber.ramp) {
        this.setRampDurationForFiber(fiber, dur + fiber.effectiveRate * (this.now - fiber.ramp.observedBegin));
    }
};
