...
Fiber.prototype.loop = function(f, delegate) {
    return this.spawn(fiber => fiber.
        call(fiber => { fiber.loopDelegate = extend(delegate, { iterationCount: 0 }); }).
        call(fiber => {
            try {
                if (fiber.loopDelegate.repeatShouldEnd?.(fiber.loopDelegate.iterationCount, fiber)) {
                    fiber.ip = fiber.fiber.length;
                    return;
                }
            } catch (error) {
                fiber.errorWithMessage
            }
        }).
        append(f).
        call(fiber => {
