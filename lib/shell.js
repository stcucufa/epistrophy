...
Fiber.prototype.rate = function(rate) {
    ...
    console.assert(rate > 0 && isFinite(rate));
    return this.call((fiber, scheduler) => { scheduler.setRateForFiber(fiber, rate); });
};
    ...
    this.seq(fiber => fiber.
        call((fiber, scheduler) => {
            if (fiber.value.length > 0) {
                scheduler.attachFiberWithValue(fiber, body, fiber.value[0]);
            }
        }).
        join({
            fiberWillJoin() {
            ...
            childFiberDidJoin(child, scheduler) {
                ...
                scheduler.attachFiberWithValue(fiber, body, fiber.value[this.i]);
            }
        })
    );
    if (typeof f === "function") {
