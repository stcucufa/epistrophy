...
Fiber.prototype.loop = function(f, delegate) {
    this.spawn(fiber => fiber.
        ...
        call(fiber => {
            ...
            try {
                if (fiber.loopDelegate.repeatShouldEnd?.(fiber)) {
                    fiber.ip = end;
                }
            } catch (error) {
    ...
    return delegate?.repeatShouldEnd ? this.join(Loop) : this;
};

const Loop = {
    fiberWillJoin(fiber) {
        this.loopFiber = fiber.children.at(-1);
    },
    finalChildFiberDidJoin(child) {
        if (child !== this.loopFiber) {
            return;
        }
        delete child.loopDelegate;
...
// Loop forever, but still joins.
export const Forever = { repeatShouldEnd: K(false) };
