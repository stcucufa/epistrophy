...
const proto = {
    ...
    runForward(scheduleItem) {
        ...
        console.assert(this.executionMode === Do || this.executionMode === Redo);
        ...
        while (this.pc < n && !this.yielded) {
            ...
            const nextOp = thread.ops[this.pc];
            const newValue = !op[Attrs].effect && !(nextOp?.[Attrs].effect && nextOp?.[Attrs].dur === 0);
            ...
            // When a value op is follow by an effect or a non-instant op then
            // the value is pushed to the thread.
            if (this.executionMode === Do && newValue) {
