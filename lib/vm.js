...
const proto = {
    ...
    runForward(scheduleItem) {
        ...
        if (this.spawns.length > 0) {
            ...
            const spawnedItems = this.spawns.map(
                spawn => this.scheduler.scheduleSpawn(spawn, this.t, this.value, this.yielded)
            );
            for (const spawnedItem of spawnedItems) {
                this.runForward(spawnedItem);
            }
        }
        ...
        if (this.yielded) {
            ...
            // The thread reached its end and may join its parent.
            this.scheduler.scheduleBackward(thread, this.t, this.pc, this.value);
            if (scheduleItem.parent) {
                const parent = scheduleItem.parent;
                const index = parent.spawns.indexOf(scheduleItem);
                parent.spawns[index] = null;
                parent.values[index] = this.value;
                if (parent.spawns.every(x => x === null)) {
                    this.scheduler.scheduleForward(parent.thread, this.t, parent.pc, parent.values);
                    parent.stale = true;
                }
            }
        }
