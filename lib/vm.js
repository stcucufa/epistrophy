...
const proto = {
    ...
    saveProperty(object, property) {
        ...
        // console.log(`+++ Saved property ${property}: [${values.join(", ")}]`);
    },

    restoreProperty(object, property) {
        object[property] = this.values.get(object)[property].pop();
        // console.log(`--- Restored property ${property}: [${this.values.get(object)[property].join(", ")}]`);
    },
    ...
    updateForward(from, to) {
        ...
        while (this.scheduler.hasFuture && this.scheduler.nextFutureTime < to) {
            console.log(`[${from}, ${to}[ >>> ${this.scheduler.dump()}`);
    ...
    runForward(scheduleItem) {
        ...
        while (this.pc < n && !this.yielded) {
            // console.log(`${this.t}/${this.pc}`, thread.ops[this.pc][this.executionMode]);
    ...
    updateBackward(from, to) {
        ...
        while (this.scheduler.hasPast && this.scheduler.nextPastTime >= to) {
            console.log(`[${to}, ${from}[ <<< ${this.scheduler.dump()}`);
    ...
    yield(thread) {
        ...
        console.log(`!!! Yield ${this.t}/${this.pc}`);
