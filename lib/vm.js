...
import { Scheduler, Do, Undo, Redo, Attrs, Error } from "./scheduler.js";
...
const proto = {
    ...
    spawnAt(t) {
        ...
        const thread = Thread();
        this.scheduler.scheduleForward(thread, t, 0, 0);
        thread.begin = t;
    ...
    unpackScheduleItem(scheduleItem) {
        ...
        this.vp = scheduleItem.vp;
        this.newValue = scheduleItem.newValue;
        this.value = this.currentThread.values[this.vp - 1];
    ...
    runForward(scheduleItem) {
        ...
        while (this.pc < n && !this.yielded) {
            ...
            const shouldPushValue = this.newValue && (op[Attrs].effect || op[Attrs].dur > 0);
            this.newValue = !op[Attrs].effect;

            if (this.executionMode === Do) {
                notify(this, "op", { thread, op, t: this.t });
            } else if (shouldPushValue) {
                console.assert(this.executionMode === Redo);
                // A value was pushed at this point so move forward in the
                // values stack.
                console.assert(this.vp < thread.values.length);
                this.value = thread.values[this.vp++];
            }
            ...
            if (this.executionMode === Do && shouldPushValue) {
                this.pushValue();
            }
        }
        ...
        if (this.yielded) {
            ...
            this.scheduler.scheduleBackward(thread, this.t, this.pc, this.vp);
            if (this.executionMode === Do) {
                if (this.newValue) {
                    this.pushValue();
                }
    ...
    // When a value op is follow by an effect or a non-instant op then the
    // value is pushed to the thread.
    pushValue() {
        console.assert(this.vp === this.currentThread.values.length);
        this.vp += 1;
        this.currentThread.values.push(this.value);
    },
    ...
    delay(thread, t, resolved = false) {
        const scheduleItem = this.scheduler.scheduleForward(thread, t, this.pc, this.vp);
        if (scheduleItem) {
            this.yielded = scheduleItem;
            this.scheduler.scheduleBackward(thread, this.t, this.pc - 1, this.vp);
            if (resolved) {
    ...
    spawnChild(thread, value) {
        ...
        this.scheduler.scheduleForward(thread, this.t, 0, thread.values.length);
    },
