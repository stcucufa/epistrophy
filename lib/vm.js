...
const proto = {
    ...
    unpackScheduleItem(scheduleItem) {
        ...
        if (this.ramps.has(this.currentThread)) {
            this.ramps.delete(this.currentThread);
        }
    ...
    updateForward(from, to) {
        ...
        this.t = to;
        for (const [thread, { f, begin, dur, forward }] of this.ramps) {
            this.currentThread = thread;
            const p = (to - begin) / dur;
            f(forward ? p : 1 - p, to - thread.begin);
        }
        this.didUpdate();
    },
    ...
    // Ramp from 0 to 1 over the given duration (stay at 0 when the duration is
    // indefinite), calling f at the end of every update.
    ramp(thread, f, dur, from, to) {
        console.assert(dur > 0);
        this.delay(thread, this.t + dur);
        this.ramps.set(thread, { f, begin: this.t, dur, from, to });
    },
