...
const proto = {
    ...
    // Pause the clock. If `normal` is false, then the clock moves to an
    // abnormal state.
    pause(normal = true) {
        if (this.broken) {
            throw Error("Broken clock");
        ...
        if (this.running && !this.paused) {
            ...
            notify(this, "pause", { broken: this.broken });
        } else if (this.broken) {
            notify(this, "pause", { broken: this.broken });
        }
    },
    ...
    // Get the execution mode for this update, updating the end time as well if
    // necessary.
    executionMode(from, to) {
        const recording = to > this.endTime;
        if (recording) {
            this.endTime = to;
        }
        return recording ? Do : (to > from ? Redo : Undo);
    },
    ...
    update() {
        ...
        this.lastUpdate = to;
        if (to !== from) {
            notify(this, "update", { from, to, mode: this.executionMode(from, to) });
            if (to === 0) {
                this.stop();
            }
        }
