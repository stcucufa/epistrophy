...
export default class Fiber {
    ...
    effect(f) {
        ...
        }).meta({ op: "effect", async: isAsync(f) });
    }
    ...
    ramp(dur, delegate) {
        if (typeof dur === "function") {
            ...
            this.ops.push(function(scheduler) { scheduler.endRampForFiber(this); });
            this.meta({ op: "ramp/end" });
        } else {
            const effectiveDur = typeof dur === "string" ? parseOffsetValue(dur) : dur;
            if (typeof dur === "number" && dur > 0) {
                this.ops.push(function(scheduler) { scheduler.beginRampForFiber(this, effectiveDur, delegate); });
