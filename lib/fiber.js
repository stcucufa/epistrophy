    ...
    each(f) {
        return this.repeat(f, {
            repeatShouldEnd(i, fiber) {
                if (i === 0) {
                    this.value = fiber.value;
                    const type = this.error ? "error" : typeOf(fiber.value);
                    this.values =
                        type === "array" ? fiber.value :
        ...
        this.ops.push(function(scheduler) {
            ...
            if (type === "array" || type === "set") {
                ...
                for (const [key, value] of values) {
                    const child = this.attach(scheduler).name(key);
                    child.ops = template.ops;
                    child.result = { value };
                }
            } else {
