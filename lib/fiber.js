...
export default class Fiber {
    ...
    repeat(f, delegate = {}) {
        return repeat.call(this, f, Object.assign(Object.create(delegate), {
            repeatShouldEnd(count, fiber, scheduler) {
                // FIXME 4F01 Delegates should be instantiated
                return fiber.error || delegate.repeatShouldEnd?.call(delegate, this.count, fiber, scheduler);
            }
        }));
    }

    // Retry is like repeat except that it ends when the fiber has an error.
    retry(f, delegate = {}) {
        return repeat.call(this, f, Object.assign(Object.create(delegate), {
            repeatShouldEnd(count, fiber, scheduler) {
                // FIXME 4F01 Delegates should be instantiated
                return (count > 0 && !fiber.error) ||
                    delegate.repeatShouldEnd?.call(delegate, this.count, fiber, scheduler);
            }
        }));
    }
...
// The base repeat function for repeat and retry. Note that the delegate object
// that was passed by the user is the prototype of `delegate`.
function repeat(f, delegate) {
    this.ops.push(() => {
        if (this.error) {
        ...
        if (this.repeatDelegate) {
            delegate.parent = this.repeatDelegate;
        }
        this.repeatDelegate = delegate;
    ...
    f(this);
    this.ops.push(scheduler => {
        if (!Object.getPrototypeOf(this.repeatDelegate).repeatShouldEnd &&
            scheduler.now === this.repeatDelegate.iterationTime) {
