...
export default class Fiber {
    ...
    cancel(scheduler) {
        ...
        if (this.handleError.at(-1)) {
            if (this.joinDelegate) {
                for (const fiber of this.joinDelegate.pending) {
    ...
    exec(f) {
        this.ops.push(isAsync(f) ? function(scheduler) {
    ...
    // FIXME 4J09 Spawn a fiber for repeat
    each(f) {
        return this.
            spawn(fiber => fiber.
                exec(fiber => {
        ...
        this.ops.push(function(scheduler) {
            ...
            if (type === "array" || type === "set") {
                for (const value of values) {
                    const child = this.attach(scheduler);
                ...
                this.attach(scheduler, template);
            }
        });
        return this;
    }
