...
export default class Fiber {
    ...
    join(delegate = {}) {
        this.ops.push(scheduler => {
            if (this.error) {
                // FIXME what to do about pending children?
                if (this.pendingChildren?.length > 0) {
                    console.warn(`Not joinining because of error; pending children: ${this.pendingChildren.size}`);
                }
                return;
            }
            console.assert(!this.joinDelegate);
            if (this.pendingChildren) {
                console.assert(this.pendingChildren.length > 0);
                this.joinDelegate = Object.create(delegate);
    ...
    childDidEnd(fiber, scheduler) {
        const index = this.pendingChildren?.indexOf(fiber);
        if (!(index >= 0)) {
            return;
        }
        this.pendingChildren.splice(index, 1);
        this.joinDelegate?.childFiberDidEnd?.call(this.joinDelegate, fiber, index, scheduler);
        if (this.pendingChildren.length === 0) {
...
export const All = {
    fiberWillJoin(fiber) {
        this.values = new Array(fiber.pendingChildren.length);
    },

    childFiberDidEnd(child, index) {
        this.values[index] = child.value;
        if (child.parent.pendingChildren.length === 0) {
