...
export default class Fiber {
    ...
    childDidEnd(fiber, scheduler) {
        ...
        if (fiber.error && typeof this.joinDelegate.childFiberDidEndInError === "function") {
            this.joinDelegate.childFiberDidEndInError.call(this.joinDelegate, fiber, scheduler);
        } else {
...
export const All = {
    ...
    childFiberDidEndInError({ parent: fiber, error }, scheduler) {
        cancelSiblings(this, scheduler);
        fiber.result.error = error;
        delete this.values;
    },
...
export const First = (useValue = true) => ({
    childFiberDidEndInError({ parent, error }) {
        if (this.pending.size === 0) {
            parent.result.error = error;
        }
    },

    childFiberDidEnd({ parent, value }, scheduler) {
        cancelSiblings(this, scheduler);
        if (useValue) {
            parent.value = value;
        }
