...
import { add, create, html, svg, timecount } from "./util.js";
...
const proto = {
    ...
    *tracks() {
    ...
    op(thread, op, t) {
        ...
        if (!op) {
            track.items.appendChild(svg("circle", { class: "end", cx: track.x, cy: track.y, r: R }));
            track.items.appendChild(svg("circle", { class: "end", cx: track.x, cy: track.y, r: R / 2}));
            if (thread.parent) {
                const y2 = this.tracksById[thread.parent.id].y;
