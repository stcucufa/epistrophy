    ...
    op(thread, op, t) {
        ...
        if (!op) {
            ...
            if (dur === 0) {
                ...
                item = track.items.appendChild(asyncOpElement(track, op));
                item.classList.toggle("effect", !!effect);
                if (dur === null) {
...
// Make a rect for an async op, except for ramp which has a triangular shape.
function asyncOpElement(track, op) {
    if (op[Attrs].tag === "ramp") {
        return svg("path", { d: `M${track.x},${track.y + R}h${ASYNC_W}v${-2 * R}z` });
    }
    return svg("rect", {
        x: track.x, y: track.y - R, width: ASYNC_W, height: 2 * R
    });
}
