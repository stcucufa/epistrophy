...
// Default duration value for delays.
export const Instant = Symbol();
...
export class Scheduler extends EventTarget {
    ...
    // Set the effective rate (the product of the fiber’s own rate and its
    // parent’s effective rate) of a fiber. This may pause or resume the fiber,
    // or affect an ongoing ramp.
    setEffectiveRateForFiber(fiber, effectiveRate, now) {
        ...
        if (effectiveRate === 0) {
            ...
            const delta = now - fiber.pausedTime;
            fiber.observedBegin += delta;
            if (fiber.delay) {
                fiber.delay.observedBegin += delta;
                this.rescheduleFiber(fiber, fiber.delay.observedBegin + fiber.delay.observedDur);
            }
            // FIXME 5F01 Kernel: unramp
            if (fiber.ramp) {
                fiber.ramp.observedBegin += delta;
                this.rescheduleFiber(fiber, fiber.ramp.observedBegin + fiber.ramp.observedDur);
            }
            ...
            if (fiber.delay) {
                const delay = fiber.delay;
                if (isFinite(delay.dur)) {
                    delay.observedDur = delay.dur / effectiveRate;
                } else if (infiniteRate) {
                    // TODO
                }
                delay.observedBegin = now - r * (now - delay.observedBegin);
                this.rescheduleFiber(fiber, delay.observedBegin + delay.observedDur);
            }
            if (fiber.ramp) {
                ...
                if (isFinite(ramp.dur)) {
                    ...
                    ramp.elapsed = fiber.effectiveRate * (now - ramp.observedBegin);
                    ramp.observedDur = 0;
                }
    ...
    // FIXME 5G03 Kernel: setDelayDurationForFiber
    ...
    update(begin, end) {
        while (this.instants.length > 0 && this.instants[0] >= begin && this.instants[0] < end) {
            ...
            while (queue.length > 0) {
                ...
                this.instantsByFiber.delete(fiber);
                if (fiber.effectiveRate === 0) {
                    continue;
                }
                this.spawns = [];
                this.joins = [];
                if (Object.getOwnPropertySymbols(fiber.scope).length > 0 || Object.keys(fiber.scope).length > 0) {
                    fiber.scope = Object.create(fiber.scope);
                }
                if (fiber.delay) {
                    fiber.now += fiber.delay.dur;
                    delete fiber.delay;
                }
                if (fiber.ramp) {
                    ...
                    if (!isFinite(fiber.effectiveRate) && !isFinite(ramp.dur)) {
                        fiber.now = Infinity;
                    }
                }
                // Run the current fiber until it ends or yields.
                if (fiber.runToCompletion()) {
                    this.fibers.delete(fiber);
                    if (fiber.effectiveRate > 0 && fiber.parent?.childFiberDidEnd(fiber)) {
                        this.joins.push(fiber.parent);
                    }
                }
...
export class Fiber extends Array {
    ...
    // Create a delay of a given duration (a literal value, which can be a
    // number of 0 or more milliseconds or the special value `Instant`; or a
    // function returning such a literal). The default duration of a delay is
    // Instant.
    delay(dur = Instant) {
        return this.op("delay", dur);
    }
    ...
    static Ops = {
        ...
        // Get the effective duration of the delay and yield after scheduling
        // the fiber again if the duration is valid. Do nothing if the observed
        // duration ends up being 0 (either because the duration is zero or the
        // fiber has an infinite rate), and throw if the duration is invalid.
        delay(dur) {
            let effectiveDur = this.getEffectiveParameter(dur);
            if (effectiveDur === Instant) {
                effectiveDur = this.effectiveRate * (this.scheduler.end - this.scheduler.now);
            }
            if (effectiveDur >= 0) {
                const observedDur = isFinite(this.effectiveRate) ? effectiveDur / this.effectiveRate : 0;
                if (observedDur === 0) {
                    // The delay happens synchronously.
                    this.now += effectiveDur;
                    return;
                }
                this.delay = { dur: effectiveDur, observedDur, observedBegin: this.scheduler.now };
                this.scheduler.scheduleFiber(this, this.scheduler.now + observedDur);
                return true;
            }
            throw Error(`Effective duration for delay is invalid (${effectiveDur})`);
        },
        ...
        join(delegate) {
            if (this.children) {
                this.joinDelegate = extend(delegate, { begin: this.now, observedBegin: this.scheduler.now });
...
// Symbol key for value and error in scope.
...
export class ScheduledFiber {
    ...
    // is empty or is an extension of the parent scope (which initializes the
    // fiber’s trace).
    ...
    get error() {
        return this.scope?.[ErrorKey];
    }

    set error(error) {
        this.scope[ErrorKey] = error;
    }

    get value() {
        return this.scope?.[ValueKey];
    }

    set value(value) {
        this.scope[ValueKey] = value;
    }
    ...
    cancel() {
        ...
        if (!this.handlesErrors) {
            ...
            if (this.delay) {
                this.now += ((this.scheduler.now ?? this.scheduler.clock.now) - this.delay.observedBegin) /
                    this.effectiveRate;
                delete this.delay;
            }
            return true;
    ...
    childFiberDidEnd(fiber) {
        ...
        this.now = this.joinDelegate.begin + this.effectiveRate * (this.scheduler.now - this.joinDelegate.observedBegin);
    ...
    // Commit the current state of the fiber to the trace.
    commit(status) {
        this.trace.push([this.now, this.ip, this.scope]);
        return status;
    }
    ...
    // reaching an infinite now or when not moving forward anymore (i.e., when
    // paused or reversing).
    runToCompletion() {
        while (this.effectiveRate < 0 || this.ip < this.fiber.length) {
            if (!isFinite(this.now) || this.effectiveRate === 0) {
                return;
            } else if (this.effectiveRate < 0) {
                if (this.trace.length > 0) {
                    const [now, ip, scope] = this.trace.pop();
                    this.scheduler.scheduleFiber(this, this.scheduler.now + (now - this.now) / this.effectiveRate);
                    this.now = now;
                    this.ip = ip;
                    this.scope = scope;
                    return;
