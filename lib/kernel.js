...
// Index of first argument in trace.
export const Arg = 2;
    ...
    update(begin, end) {
        while (this.instants.length > 0 && this.instants[0] >= begin && this.instants[0] < end) {
            ...
            while (queue.length > 0) {
                ...
                if (this.currentFiber.effectiveRate === 0) {
                    continue;
                }
                this.spawns = [];
                this.joins = [];
...
export class Fiber extends Array {
    ...
    // Wrap synchronous function `f` in a fiber instruction with an optional
    // reverse function `g`.
    call(f, g) {
        this.push(["call", this.everDepth > 0, f]);
        if (g) {
            this.at(-1).push(g);
        }
        return this;
    ...
    static Ops = {
        ...
        ramp(dur, f) {
            ...
            if (effectiveDuration >= 0) {
                this.trace.at(-1).push(effectiveDuration);
    ...
    // Undo for instructions.
    static Unops = {
        // Undo call: call g (when provided) or restore the previous value.
        call(value, _, g) {
            this.value = g?.(this) ?? value;
        },

        // Undo ramp: still ramps in the opposite direction, using the same
        // effective duration.
        ramp(dur, f) {
            const effectiveDuration = this.trace.at(-1)[Arg];
        },
    };
}
...
export class ScheduledFiber {
    ...
    runToCompletion() {
        while (
            (this.effectiveRate >= 0 && this.ip < this.fiber.length) ||
            (this.effectiveRate < 0 && this.trace.length > 0)
        ) {
            // FIXME 560C Kernel: undoing at infinite rate
            ...
            if (this.effectiveRate > 0) {
                ...
                let yields = false;
                this.trace.push([this.now, this.ip]);
                try {
                    yields = Fiber.Ops[op].call(this, ...args);
                } catch (error) {
                    this.errorWithMessage(error);
                }
                this.trace.at(-1).push(this.value, this.error);
                if (yields) {
                    return;
                }
            } else {
                const [now, ip, value, error] = this.trace.at(-1);
                this.now = now;
                this.ip = ip - 1;
                if (error) {
                    continue;
                ...
                const [op, ever, ...args] = this.fiber[this.ip];
                Fiber.Unops[op].call(this, value, ...args);
                this.trace.pop();
            }
        }
        return true;
