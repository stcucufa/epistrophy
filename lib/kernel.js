...
export class Scheduler extends EventTarget {
    ...
    setEffectiveRateForFiber(fiber, effectiveRate) {
        ...
        if (fiber.ramp) {
            ...
            fiber.now = Infinity;
            fiber.asyncDelegate.asyncWasCancelled?.call(fiber.asyncDelegate, fiber, this);
            delete fiber.asyncDelegate;
        }
        fiber.effectiveRate = effectiveRate;
        if (fiber.child) {
            this.setEffectiveRateForFiber(fiber.child, effectiveRate * fiber.child.rate);
        }
        if (fiber.children) {
            for (const child of fiber.children) {
                this.setEffectiveRateForFiber(child, effectiveRate * child.rate);
            }
        }
    }
    ...
    static Ops = {
        ...
        await(f, delegate) {
            ...
            f(this).then(value => {
                if (this.asyncDelegate === instance) {
                    ...
                    this.value = instance.asyncWillEndWithValue?.call(instance, value) ?? value ?? this.value;
                    this.asyncDidEnd(now);
                }
            }).catch(error => {
                if (this.asyncDelegate === instance) {
                    const now = this.scheduler.asyncCallEnded(this);
                    instance.asyncWillEndWithError?.call(instance, error, this);
        ...
        event(target, type, delegate) {
            ...
            this.asyncDelegate = extend(delegate, {
                ...
                handleEvent(event) {
                    ...
                    if (this.asyncDelegate.eventShouldBeIgnored?.call(this.asyncDelegate, event, fiber)) {
                        return;
                    }
                    this.asyncDelegate.eventWasHandled?.call(this.asyncDelegate, event, fiber);
        ...
        join(delegate) {
            if (this.children) {
                ...
                this.joinDelegate.fiberWillJoin?.call(this.joinDelegate, this);
                return true;
            }
        },
...
export class ScheduledFiber {
    ...
    cancel() {
        ...
        if (!this.handlesErrors) {
            this.asyncDelegate?.asyncWasCancelled.call(this.asyncDelegate, this);
            return true;
        }
    }
    ...
    childFiberDidEnd(fiber) {
        ...
        this.joinDelegate.childFiberDidJoin?.call(this.joinDelegate, fiber);
        if (this.joinDelegate.finalChildFiberDidJoin?.call(this.joinDelegate, fiber, scheduler) ||
    ...
    // reaching an infinite now.
    runToCompletion() {
        while (this.ip < this.fiber.length && isFinite(this.now)) {
