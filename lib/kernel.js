...
export class Scheduler extends EventTarget {
    ...
    // Push a child fiber instance to be spawned immediately (before any other
    // spawns) as the only child, as used by seq.
    pushFiber(fiber, child) {
        this.spawns.unshift(fiber.child = new ScheduledFiber(child, this.now, fiber));
        if (fiber.everDepth > 0) {
            fiber.child.everDepth = fiber.everDepth;
        }
    }
    ...
    scheduleFiber(fiber, t) {
        if (fiber.__ENDED__) {
            console.warn(`!!! ${fiber.id}: Scheduling a fiber that ended already?!`);
        }
        if (fiber.ip >= 0 && this.instantsByFiber.has(fiber)) {
            console.warn(`!!! ${fiber.id}: Scheduling a fiber twice?!`);
        }
        if (isFinite(t)) {
    ...
    setRampDurationForFiber(fiber, dur) {
        ...
        fiber.ramp.dur = dur;
        const end = Math.max(this.now ?? this.clock.now, fiber.ramp.begin + dur);
        if (end === fiber.ramp.end) {
            return;
        }
        fiber.ramp.end = end;
    ...
    // a ramp object with begin, dur, end and f to the fiber. Store end to
    // manage the ramp being cutoff if duration changes during the ramp.
    scheduleRamp(fiber, dur, f) {
        const begin = this.now;
        const end = begin + dur;
        this.ramps.add(Object.assign(fiber, { p: 0, ramp: { begin, dur, end, elapsed: 0, f } }));
        f?.(fiber, this);
        if (dur > 0) {
            this.scheduleFiber(fiber, end);
            return true;
    ...
    update(begin, end) {
        ...
        for (const fiber of this.ramps.values()) {
            ...
            if (ramp.f) {
                ramp.elapsed = this.now - ramp.begin;
...
export class Fiber {
    ...
    static Ops = {
        ...
        // Push a new instance of the child fiber and yield until the child
        // joins. Previous children are not affected.
        seq(scheduler, child) {
            scheduler.pushFiber(this, child);
...
export class ScheduledFiber {
    ...
    childFiberDidEnd(fiber, scheduler) {
        if (fiber.__ENDED__) {
            console.info(`<<< ${fiber.id} ended twice`);
        }
        fiber.__ENDED__ = true;
