...
export class Scheduler extends EventTarget {
    ...
    setEffectiveRateForFiber(fiber, effectiveRate, now) {
        ...
        if (effectiveRate === 0) {
            ...
            if (fiber.ramp) {
                ...
                if (r < 0) {
                    // TODO infinite rate
                    ramp.observedDur = ramp.observedBegin - now;
                    ramp.elapsed = ramp.observedDur * fiber.effectiveRate;
                    ramp.observedBegin = 2 * now - ramp.observedBegin;
    ...
    scheduleRamp(fiber, dur, f) {
        ...
        try {
            f?.(fiber);
            const isFailing = fiber.isFailing;
            if (isFailing || observedDur === 0) {
                const definite = isFinite(dur);
                if (definite && !isFailing) {
                ...
                // the fiber is not scheduled again) when not failing.
                return !definite && !isFailing;
            }
            this.ramps.add(fiber);
            this.scheduleFiber(fiber, observedBegin + observedDur);
            return true;
        } catch (error) {
            // Fail immediately without actually scheduling the ramp.
            fiber.errorWithMessage(error);
        }
    }

    // Schedule a reverse ramp, where begin comes after the end (so that p
    // goes down from 1 to 0).
    scheduleReverseRamp(fiber, dur, f) {
        if (!(dur > 0 && f)) {
            return;
        }
        throw Error("TODO (schedule reverse ramp)");
        return true;
    }
        ...
        while (this.instants.length > 0 && this.instants[0] >= begin && this.instants[0] < end) {
            ...
            while (queue.length > 0) {
                ...
                if (fiber.ramp) {
                    ...
                    // one last time.
                    const ramp = fiber.ramp;
                    if (fiber.effectiveRate < 1) {
                        fiber.p = 0;
                        fiber.now = ramp.begin;
                        ramp.elapsed = 0;
                    } else if (isFinite(ramp.dur)) {
                        fiber.p = 1;
            ...
            if (ramp.f && fiber.effectiveRate !== 0) {
                ...
                if (p < 1) {
                    ...
                    if (fiber.isFailing) {
                        ...
                        fiber.trace.push([this.now, ramp, fiber.scope]);
                    }
                }
            }
        }
...
export class Fiber extends Array {
    ...
    // Handling of some the of the operators when running in reverse.
    static ReverseOps = {

        // Run the ramp in reverse for the previously computed effective
        // duration (as derived from the trace), provided that a callback
        // function is specified (otherwise this is simply a delay).
        ramp(effectiveDuration, f) {
            return this.scheduler.scheduleReverseRamp(this, effectiveDuration, f);
        }
    };
}
...
export class ScheduledFiber {
    ...
    // True if the fiber is failing, i.e., the fiber has an error and does not
    // handle it.
    get isFailing() {
        return !!this.error && !this.handlesErrors;
    }
    ...
    commit(status) {
        const trace = [this.now, this.ip];
        this.trace.push(trace);
        if (this.now === 0 || (Object.getOwnPropertySymbols(this.scope).length > 0 || Object.keys(this.scope).length > 0)) {
            trace.push(this.scope);
            this.scope = Object.create(this.scope);
        } else {
            trace.push(Object.getPrototypeOf(this.scope));
        }
        return status;
    }
