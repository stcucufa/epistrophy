        ...
        while (this.instants.length > 0 && this.instants[0] >= begin && this.instants[0] < end) {
            ...
            while (queue.length > 0) {
                ...
                if (this.currentFiber.ramp) {
                    ...
                    if (!isFinite(this.currentFiber.effectiveRate)) {
    ...
    static Ops = {
        ...
        await(f, delegate) {
            ...
            f(this).then(value => {
                if (this.asyncDelegate === instance) {
                    ...
                    this.value = delegate?.asyncWillEndWithValue?.call(delegate, value, this) ?? value ??
                        this.value;
                ...
                if (this.asyncDelegate === instance) {
                    const now = this.scheduler.asyncCallEnded(this);
                    delegate?.asyncWillEndWithError?.call(delegate, error, this);
        ...
        event(target, type, delegate) {
            ...
            this.asyncDelegate = extend(delegate, {
                ...
                handleEvent(event) {
                    ...
                    if (delegate?.eventShouldBeIgnored?.call(delegate, event, fiber)) {
                        return;
                    }
                    delegate?.eventWasHandled?.call(delegate, event, fiber);
        ...
        join(delegate) {
            if (this.children) {
                ...
                delegate?.fiberWillJoin?.call(delegate, this);
                return true;
            }
        },
...
export class ScheduledFiber {
    ...
    cancel() {
        ...
        if (!this.handlesErrors) {
            this.asyncDelegate?.asyncWasCancelled.call(Object.getPrototypeOf(this.asyncDelegate), this);
            return true;
        }
    }
    ...
    childFiberDidEnd(fiber) {
        ...
        delegate.childFiberDidJoin?.call(delegate, fiber);
