...
export class Scheduler extends EventTarget {
    ...
    setEffectiveRateForFiber(fiber, effectiveRate) {
        ...
        const finiteRate = isFinite(effectiveRate);
        const now = this.now ?? this.clock.now;
        ...
        if (fiber.ramp) {
            ...
            if (isFinite(ramp.dur)) {
                ...
                // An infinite fiber at infinite rate still does not finish
                // but will be immediately interrupted.
        ...
        } else if (fiber.asyncDelegate && !finiteRate) {
...
export class ScheduledFiber {
    ...
    // nothing when yielding, and true when done.
