...
export class Scheduler extends EventTarget {
    ...
    // FIXME 5H01 Kernel: update rate during delay
...
export class Fiber extends Array {
    ...
    static Ops = {
        ...
        delay(dur) {
            let effectiveDuration = this.getEffectiveParameter(dur);
            if (effectiveDuration === Instant) {
                effectiveDuration = this.effectiveRate * (this.scheduler.end - this.scheduler.now);
            }
            if (effectiveDuration >= 0) {
                const observedDuration = isFinite(this.effectiveRate) ? effectiveDuration / this.effectiveRate : 0;
                if (observedDuration === 0) {
                    // The delay happens synchronously.
                    this.now += effectiveDuration;
                    return;
                }
                this.delay = { dur: effectiveDuration, observedBegin: this.scheduler.now };
                this.scheduler.scheduleFiber(this, this.scheduler.now + observedDuration);
                return true;
            }
            throw Error(`Effective duration for delay is invalid (${effectiveDuration})`);
        },
