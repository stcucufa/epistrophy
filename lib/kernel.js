...
export class Scheduler extends EventTarget {
    ...
    setEffectiveRateForFiber(fiber, effectiveRate) {
        ...
        if (fiber.ramp) {
            ...
            if (isFinite(ramp.dur)) {
                ramp.observedDur = ramp.dur / fiber.effectiveRate;
            }
    ...
    scheduleRamp(fiber, dur, f) {
        ...
        // End the ramp synchronously with p=1.
        fiber.p = 1;
        f?.(fiber, this);
        delete fiber.p;
        delete fiber.ramp;
    ...
    update(begin, end) {
        ...
        delete this.spawns;
        delete this.joins;
        // Any fiber that yields at infite rate collapses.
        for (const fiber of this.fibers) {
            if (!isFinite(fiber.effectiveRate)) {
                fiber.collapse(this);
