...
export class Scheduler {
    ...
    attachFiber(fiber, child) {
        const scheduledChild = new ScheduledFiber(child, this.now);
        scheduledChild.parent = fiber;
        if (!fiber.children) {
        ...
        if (this.spawns) {
            ...
            this.scheduleFiber(scheduledChild, this.clock.now);
        }
    ...
    cancelFiber(fiber) {
        if (this.fibers.has(fiber) && fiber.cancel(this) && fiber !== this.currentFiber) {
            this.scheduleFiber(fiber, this.now);
        }
    }
    ...
    scheduleFiber(fiber, t) {
        ...
        const scheduledFiber = fiber.ip >= 0 ? fiber : new ScheduledFiber(fiber, this.now);
    ...
    update(begin, end) {
        ...
        for (const [fiber, ramp] of this.ramps.entries()) {
            ...
            const p = isFinite(ramp.dur) ? 0 : (this.now - ramp.begin) / ramp.dur;
            if (p < 1) {
...
export class Clock {
    ...
    get now() {
        return this.state === Stopped ? this.currentTime : performance.now() - this.startTime;
    }
