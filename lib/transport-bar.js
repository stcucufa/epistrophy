...
export class TransportBar {
    constructor() {
        ...
        this.scheduler.scheduleFiber(new Fiber().
            ramp(Infinity, fiber => {
                // Hours
                digits[0].textContent = Math.floor((fiber.now / 36_000_000) % 10);
                digits[1].textContent = Math.floor((fiber.now / 3_600_000) % 10);
                // Minutes
                digits[2].textContent = Math.floor((fiber.now % 600_000) / 100_000);
                digits[3].textContent = Math.floor((fiber.now / 60_000) % 10);
                // Seconds
                digits[4].textContent = Math.floor((fiber.now % 60_000) / 10_000);
                digits[5].textContent = Math.floor((fiber.now / 1000) % 10);
