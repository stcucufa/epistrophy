    ...
    join(storeValues = true) {
        return this.asyncdo((thread, vm) => {
            if (this.children.length === 0) {
                ...
                this.cancellable = new Set(this.children);
    ...
    childThreadDidEnd(childThread, vm) {
        ...
        if (this.values) {
            if (Array.isArray(this.values)) {
                if (this.dynamic) {
                    this.values.push(vm.value);
                } else {
                    this.values[index] = vm.value;
                }
            }
            this.cancellable.delete(childThread);
            if (--this.pending === 0) {
                delete this.ended;
                delete this.dynamic;
                for (const child of this.cancellable.values()) {
                    vm.cancel(child);
                }
