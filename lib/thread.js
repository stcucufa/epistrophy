...
const proto = {
    ...
    delay(dur) {
        ...
        }, { tag: "delay", dur: time.read(dur), effect });
    },

    // Ramp from 0 to 1 over the duration and call f at every update with the
    // ramp value.
    ramp(f, dur) {
        const forward = (thread, vm) => { vm.ramp(thread, f, time.read(dur ?? vm.value), 0, 1); };
        this.ops.push(
            forward,
            (thread, vm) => { vm.ramp(thread, f, time.read(dur ?? vm.value), 1, 0); },
            forward,
            { tag: "ramp", dur: time.read(dur), effect }
        );
        return this;
    },
