...
const Ops = {
    ...
    // `fiberWillJoin` with the delegate as `this`. When there are no children
    // to join, end synchronously.
    beginjoin(scheduler, delegate) {
        if (!this.children) {
            return;
        }
        console.assert(this.children.length > 0);
        this.joinDelegate = extend(delegate, { begin: this.now, pending: new Set(this.children) });
    ...
    spawn(scheduler, child) {
        if (this.rate > 0) {
            const instance = scheduler.attachFiber(this, Object.create(child));
...
export class Scheduler {
    ...
    // The fiber is not active anymore. Notify the parent since it may be
    // joining, and as a result need to be scheduled when the last child has
    // ended.
    fiberEnded(fiber) {
    ...
    setFiberRate(fiber, rate, setOwnRate = true) {
        ...
        if (effectiveRate < 0) {
            for (const [child, begin] of fiber.spawns.entries()) {
                if (!this.instantsByFiber.has(child)) {
                    const end = begin + child.now;
                    this.scheduleFiber(child, this.now - (fiber.now - end) / effectiveRate);
                }
            }
        }
    }
