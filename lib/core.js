...
export class Scheduler {
    ...
    setFiberRate(fiber, rate, setOwnRate = true) {
        ...
        if (fiber.children) {
            ...
            // the descendants rate (order does not matter) and unend them if
            // reversing (order does matter).
            const queue = fiber.children.slice();
            const descendants = [];
            while (queue.length > 0) {
                const child = queue.shift();
                this.setRateForSingleFiber(child, child.parent.rate * child.ownRate);
            ...
            if (effectiveRate < 0) {
                ...
                for (const descendant of descendants) {
                    if (!this.instantsByFiber.has(descendant)) {
                        const { spawns, rate, now } = descendant.parent;
                        const begin = spawns.get(descendant);
                        const end = begin + descendant.now;
                        this.scheduleFiber(descendant, this.now - (now - end) / rate);
                    }
                }
            }
        }
