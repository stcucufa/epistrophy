...
export class Scheduler {
    ...
    // ever() block. Finally, children are cancelled.
    cancelFiber(fiber) {
        console.assert(fiber.rate > 0);
        console.assert(!Object.hasOwn(fiber, "observedEnd"));
        fiber.error = Fiber.Cancelled;
        if (fiber.ever === 0) {
            if (fiber.asyncDelegate) {
                fiber.now += (this.clock.now - fiber.asyncDelegate.observedBegin) * fiber.rate;
    ...
    endRampForFiber(fiber, p) {
        ...
        console.assert(isFinite(dur) || p === 0);
        console.log(`endRampForFiber: observedEnd=${observedEnd}, now=${this.now}`);
