...
export class Fiber {
    ...
    *run(scheduler) {
        if (!Object.hasOwn(this, "now")) {
            ...
            this.ownRate = 1;
            this.parent?.childFiberDidBegin(this);
        }
        if (this.rate === 0) {
            yield;
        }
    ...
    childFiberDidBegin(child) {
        console.info(`BEGIN at t=${this.now}: ${child.id} < ${this.id}`);
        if (!this.spawns.has(this.now)) {
            this.spawns.set(this.now, [child]);
        } else {
            this.spawns.get(this.now).unshift(child);
        }
    }
...
export class Scheduler {
    ...
    setFiberRate(fiber, rate, setOwnRate = true) {
        ...
        if (effectiveRate < 0) {
            for (const [begin, children] of fiber.spawns.entries()) {
