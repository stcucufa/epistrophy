...
const Ops = {
    ...
    // `fiberWillJoin` with the delegate as `this`. When there are no 
...
export class Scheduler {
    ...
    setRampDurationForFiber(fiber, dur) {
        ...
        const ramp = this.ramps.get(fiber);
        if (dur === ramp.dur || ramp.observedEnd <= this.now) {
            return;
        }
        ramp.observedDur = dur / fiber.rate;
        console.assert(ramp.observedDur >= 0);
        ramp.observedEnd = Math.max(ramp.observedBegin + ramp.observedDur, this.now);
