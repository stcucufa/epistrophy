...
export class Scheduler {
    ...
    // begin time, end time and duration, its actual duration (in local time),
    // and the progress callback f. When the ramp is paused (because the fiber
    // rate is zero), then keep track of the remaining time for resuming.
    beginRampForFiber(fiber, dur, f) {
        ...
        this.ramps.set(fiber, { observedBegin, observedDur, observedEnd, dur, f });
        if (observedDur > 0) {
