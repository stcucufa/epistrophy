...
const compareTimes = (a, b) => time.cmp(a.t, b.t) || (a.thread.spawnID - b.thread.spawnID);

const proto = {
    init() {
        this.futureQueue = Queue(compareTimes);
        this.pastQueue = Queue((a, b) => time.cmp(b.t, a.t) || (b.thread.spawnID - a.thread.spawnID));
        this.unresolved = new Map();
    },

    // Clear the future queue, rescheduling the first occurrence of every
    // thread at the same time with the Do execution mode.
    reset() {
        const rescheduled = new Map();
        for (const item of this.futureQueue) {
            const id = item.thread.spawnID;
            if (!rescheduled.has(id) || rescheduled.get(id).t > item.t) {
                rescheduled.set(id, item);
            }
        }
        for (const [thread, pc] of this.unresolved.entries()) {
            if (!rescheduled.has(thread.spawnID)) {
                rescheduled.set(thread.spawnID, { thread, t: time.unresolved, pc });
            }
        }
        this.futureQueue = Queue(compareTimes);
        this.unresolved = new Map();
        for (const { thread, t, pc, value } of rescheduled.values()) {
            const n = thread.ops.length;
            const op = thread.ops[(pc + n - 1) % n];
            this.scheduleForward(thread, time.isUnresolved(op[Attrs].dur) ? time.unresolved : t, pc, value);
        }
    },
