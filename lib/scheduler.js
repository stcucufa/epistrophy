...
export default class Scheduler {
    ...
    fiberLocalTime(fiber, now) {
        return fiber.now + (
            isFinite(fiber.rate) ? ((now ?? this.now) - (fiber.lastUpdateTime ?? fiber.beginTime)) * fiber.rate : 0
        );
    }
    ...
    attachFiber(fiber, child, forward = true) {
        if (!child) {
            child = new Fiber();
        }
        child.parent = fiber;
        ...
        this.resetFiber(child, forward);
        this.resumeFiber(child);
        return child;
    }

    // Undo attaching a fiber, meaning that the fiber ended its undo run.
    unattachFiber(child) {
        console.assert(child.unip === 0);
        const fiber = child.parent;
        remove(fiber.children, child);
    ...
    fiberEnded(fiber) {
        ...
        if (Fiber.Names.has(fiber)) {
            const name = this.removeNameForFiber(fiber);
            fiber.unops.push(function(scheduler) {
                scheduler.setNameForFiber(fiber, name);
            });
        }
        ...
        parent?.unops.push(function(scheduler) {
            scheduler.attachFiber(parent, fiber, false);
        });
    ...
    setRateForFiber(fiber, rate, setOwnRate = true) {
        ...
        if (reverse) {
            fiber.unip = fiber.unops?.length ?? 0;
        }
        if (setOwnRate) {
            fiber.ownRate = rate;
        }
    ...
    // When the fiber resumes from a delay, set its local time and add the
    // reverse delay to its undo stack.
    fiberResumedAfterDelay(fiber) {
        const { dur, fiberEnd } = this.delays.get(fiber);
        fiber.now = fiberEnd;
        this.delays.delete(fiber);
        fiber.unops.push(function(scheduler) {
            scheduler.setDelayForFiber(this, -dur);
            if (!this.yielded) {
                this.now -= dur;
            }
        });
    ...
    update(begin, end) {
        ...
        while (this.instants.length > 0 && this.instants[0] >= begin && this.instants[0] < end) {
            ...
            while (queue.length > 0) {
                ...
                this.instantsByFiber.delete(fiber);
                if (this.delays.has(fiber)) {
                    this.fiberResumedAfterDelay(fiber);
                }
