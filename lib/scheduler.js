...
const proto = {
    ...
    valueOf(thread) {
        ...
        return thread.values[past.sort((a, b) => time.cmp(b.t, a.t) || (b.pc - a.pc))[0]?.vp];
    },
    ...
    scheduleForward(thread, t, pc, vp, executionMode = Do, newValue = false) {
        if (time.isDefinite(t)) {
            return this.futureQueue.insert({ thread, t, pc, vp, executionMode, newValue });
        }
        if (time.isUnresolved(t)) {
            console.assert(executionMode === Do);
            add(this.unresolved, thread, pc);
            return { thread, t, pc, vp, executionMode, newValue };
        }
    },
    ...
    wake(thread, t, value, executionMode = Do) {
        ...
        return this.scheduleForward(thread, t, pc, value, executionMode, true);
    },
