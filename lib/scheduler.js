import { message, nop, on, PriorityQueue, remove } from "./util.js";
...
export default class Scheduler {
    constructor() {
        ...
        // Handle errors in event handlers. Fibers will register themselves when
        // entering event handlers so that we can match the error to the right
        // scheduler and fiber when they occur, or ignore them.
    ...
    cancelFiber(fiber, t) {
        t = t ?? (this.isUpdating ? this.now : this.clock.now);
        ...
        if (fiber.ever === 0) {
            this.cancelSingleFiber(fiber, t);
        }
        if (fiber.children) {
            for (const child of fiber.children) {
                this.cancelFiber(child, t);
            }
        }
    }

    // Cancel the fiber proper, which means interrupting a join, an async call,
    // or a ramp in progress, and (re-)scheduling it. This has no effect if the
    // fiber is paused, but this should also be called once the fiber resumes.
    // Don’t cut a join delegate short, but just let it end naturally (its
    // child fibers will be cancelled).
    cancelSingleFiber(fiber, t) {
        if (fiber.rate === 0 || !this.ramps.has(fiber) || fiber.joinDelegate) {
            return;
        }
        const ramp = this.ramps.get(fiber);
        this.ramps.delete(fiber);
        const { begin, dur, observedBegin, observedDur } = ramp;
        const elapsed = t - observedBegin;
        if (isFinite(observedDur)) {
            const p = elapsed / observedDur;
            fiber.now = begin + p * dur;
            fiber.trace.at(-1).push(dur, p);
        } else {
            const dur = elapsed * fiber.rate;
            fiber.now = begin + dur;
            fiber.trace.at(-1).push(dur, 0);
            if (fiber.asyncDelegate) {
                const delegate = Object.getPrototypeOf(fiber.asyncDelegate);
                delete fiber.asyncDelegate;
                delegate.asyncWasCancelled?.call(delegate, fiber, this);
            }
        }
        this.rescheduleFiber(fiber, t);
    }
    ...
    // Update the fiber local time when an async call ended.
    fiberAsyncUpdate(fiber, registerEventHandler = false) {
        console.assert(fiber.rate >= 0);
        const now = this.clock.now;
        const { begin, observedBegin } = this.ramps.get(fiber);
        fiber.now = begin + (now - observedBegin) * fiber.rate;
    ...
    setRateForSingleFiber(fiber, rate) {
        ...
        // The fiber was cancelled while it was paused, so finish cancelling
        // it now that it has resumed.
        if (fiber.rate === 0 && rate > 0 && fiber.error === Fiber.Cancelled) {
            fiber.rate = rate;
            this.cancelSingleFiber(fiber);
            return;
        }
        if (this.ramps.has(fiber)) {
            ...
            if (rate === 0) {
                ...
                if (rate < 0) {
                    fiber.asyncDelegate?.asyncWasCancelled?.(fiber, this);
                }
            } else if (rate < 0) {
    ...
    // backward) immediately. Return Yield if the fiber needs to yield.
    // A ramp stores its local begin time and duration as well as its observed
    // begin time, end time and duration, and the progress callback f. The
    // observed duration is negative when the ramp goes backward (in which
    // case observedBegin > observedEnd).
    beginRampForFiber(fiber, dur, f = nop, p = 0) {
        console.assert(!this.ramps.has(fiber));
        ...
        if (observedDur <= 0 && isFinite(dur) && p === 0) {
            // An interrupted infinite ramp going backward will have a finite
            // dur (the elapsed time to go back to), zero progress, and
            // negative rate. The backward ramp should still have an infinite
            // dur so that it always gets updated with p=0.
            const begin = fiber.now - dur;
            const observedBegin = this.now - observedDur;
            if (observedDur < 0) {
                this.ramps.set(fiber, { begin, dur: Infinity, observedBegin, f });
                this.scheduleFiber(fiber, observedBegin);
                return Fiber.Yield;
            }
        }
    ...
    // the final p value. The ramp duration may have been cutoff, in which case
    // the fiber local time should not be set to the past but match the current
    // global time.
    endRampForFiber(fiber) {
        const ramp = this.ramps.get(fiber);
        this.ramps.delete(fiber);
        const { f, begin, dur, observedBegin, observedEnd } = ramp;
        if (isFinite(dur)) {
            const p = ramp.p ?? (fiber.rate > 0 ? 1 : 0);
    ...
    update(begin, end) {
        ...
        while (this.instants.length > 0 && this.instants[0] >= begin && this.instants[0] < end) {
            ...
            while (queue.length > 0) {
                ...
                if (!this.fibers.has(fiber)) {
                    // The fiber has not begun yet.
                    this.fibers.set(fiber, fiber.run(this));
                } else if (this.ramps.has(fiber)) {
                    // End the ramp.
                    // TODO resuming from pause
                    this.endRampForFiber(fiber);
                }
        ...
        for (const [fiber, ramp] of this.ramps.entries()) {
            if (Object.hasOwn(ramp, "elapsed") || !ramp.f) {
                // Skip paused/unresolved duration ramps.
                continue;
            }
            if (isFinite(ramp.dur)) {
                ...
                fiber.now = ramp.begin + (this.now - ramp.observedBegin) * fiber.rate;
                ramp.f(0, fiber, this);
            }
        }
        ...
        message(this, "update", { begin, end, idle: this.fibers.size === 0 && this.clock.idle });
