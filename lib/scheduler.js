...
const proto = {
    ...
    scheduleForward(thread, t, pc, value) {
        if (time.isDefinite(t)) {
            return this.futureQueue.insert({ thread, t, pc, value });
        }
        if (time.isUnresolved(t)) {
            add(this.unresolved, thread, pc);
            return { thread, t, pc, value };
        }
    },
    ...
    dump() {
        const item = ({ thread, t, pc, value }) => `(T${thread.id}, ${t}, ${pc}, ${value})`;
