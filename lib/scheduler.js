...
export default class Scheduler {
    ...
    setDelayForFiber(fiber, dur) {
        ...
        if (effectiveDur > 0) {
            ...
            this.delays.set(fiber, {
                begin, dur, rate,
                fiberBegin: rate > 0 ? fiber.now : fiber.now + dur,
                fiberEnd: rate > 0 ? fiber.now + dur : fiber.now
            });
            fiber.yielded = true;
        }
    }
    ...
    endRampForFiber(fiber) {
        ...
        const { delegate, fiberEnd, dur } = this.ramps.get(fiber);
        ...
        while (this.instants.length > 0 && this.instants[0] >= begin && this.instants[0] < end) {
            ...
            while (queue.length > 0) {
                ...
                if (this.delays.has(fiber)) {
                    ...
                    fiber.now = delay.rate > 0 ? delay.fiberEnd : delay.fiberBegin;
