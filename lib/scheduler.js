...
export default class Scheduler {
    ...
    // Get the local time in an active fiber.
    fiberLocalTime(fiber) {
        if (fiber.rateChange) {
            const [t, offset] = fiber.rateChange;
            return offset + (this.now - t) * fiber.rate;
        }
        return (this.now - fiber.beginTime) * fiber.rate;
    }
    ...
    setRateForFiber(fiber, rate, setOwnRate = true) {
        ...
        fiber.rateChange = [this.now, this.fiberLocalTime(fiber)];
        fiber.rate = rate;
        if (this.delays.has(fiber)) {
            update(this.delays.get(fiber));
        } else if (this.ramps.has(fiber)) {
