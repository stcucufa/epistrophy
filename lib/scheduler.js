...
export default class Scheduler {
    ...
    setDelayForFiber(fiber, dur) {
        ...
        if (effectiveDur > 0) {
            ...
            this.delays.set(fiber, { begin, dur, rate, now: fiber.now + dur });
            fiber.yielded = true;
        }
    }
    ...
    beginRampForFiber(fiber, dur, delegate) {
        ...
        const rate = fiber.rate;
        this.ramps.set(fiber, { delegate, begin, dur, rate, localBegin: fiber.now, now: fiber.now + dur });
        fiber.yielded = true;
    ...
    endRampForFiber(fiber) {
        ...
        const { delegate, now } = this.ramps.get(fiber);
    ...
    updateDelayForFiber(fiber, dur) {
        const update = delay => {
            delay.dur = dur;
            delay.now = Math.max(fiber.now + dur, this.fiberLocalTime(fiber));
    ...
    setRateForFiber(fiber, rate, setOwnRate = true) {
        ...
        if (this.delays.has(fiber)) {
            ...
            if (this.currentFiber === fiber) {
                ...
                this.delays.set(fiber, { begin: this.now, dur: 0, rate: 0, p: 0, now: fiber.now });
            }
            fiber.yielded = true;
        }
        if (fiber.children) {
    ...
    update(begin, end) {
        ...
        while (this.instants.length > 0 && this.instants[0] >= begin && this.instants[0] < end) {
            ...
            while (queue.length > 0) {
                ...
                this.instantsByFiber.delete(fiber);
                if (this.delays.has(fiber)) {
                    fiber.now = this.delays.get(fiber).now;
        ...
        this.now = end;
        for (const [fiber, { delegate, begin, dur, rate, localBegin }] of this.ramps.entries()) {
